    ...
    window.SharedToolbarApi = {
        create: function (context) {
            return {
                ...
                setIcon: function (key, value) { var item = context.buttons[key], icon = item && item.querySelector(".sirk-toolbar-icon"), text = String(value || ""); if (!icon) return; if (text.indexOf("<svg") === 0) icon.innerHTML = text; else icon.textContent = text; },
                setBadge: function (key, value) {
                    var button = context.buttons[key]; if (!button) return;
                    var badge = button.querySelector(".sirk-toolbar-badge");
                    if (value == null || value === "") { if (badge) badge.remove(); return; }
                    if (!badge) { badge = document.createElement("span"); badge.className = "sirk-toolbar-badge"; button.appendChild(badge); }
